...
<div  class="row justify-content-center" style="margin-bottom: 40px; margin-right: 0;  margin-left: 0">
 ...
 <div style="overflow:auto; white-space: nowrap;  background-color: green;" >
         <button  style="font-size: 12px; background-color: black; color: white" class="mx-auto  font-weight-bold text-uppercase btn btn-sm   " 
         type="button">{{$total}} FCFA</button>

         <button id="dashboard_btn" style="font-size: 11px" class="mx-auto  font-weight-bold text-uppercase btn btn-sm  @if($state && $state == 'all') btn-outline-warning @else btn-outline-light @endif" 
         type="button">Tout {{$all_commands->count()}}</button>

          <button id="termine_btn" style="font-size: 11px" class="mx-auto font-weight-bold text-uppercase btn btn-sm align-middle @if($state && $state == 'termine') btn-outline-warning @else btn-outline-light @endif" type="button">Terminé {{$all_commands->where('etat', 'termine')->count()}}</button>

          <button id="enchemin_btn" style="font-size: 9px" class="mx-auto font-weight-bold text-uppercase btn btn-sm align-middle @if($state && $state == 'en chemin') btn-outline-warning @else btn-outline-light @endif" type="button">En chemin {{$all_commands->where('livreur_id', '!=', 11)->where('etat', 'en chemin')->count()}}</button>

          <button id="recupere_btn" style="font-size: 11px" class="mx-auto font-weight-bold text-uppercase btn btn-sm align-middle @if($state && $state == 'recupere') btn-outline-warning @else btn-outline-light @endif" type="button">Recuperé {{$all_commands->where('livreur_id', '!=', 11)->where('etat', 'recupere')->count()}}</button>
          
          <button id="encours_btn" style="font-size: 11px" class="mx-auto font-weight-bold text-uppercase btn btn-sm align-middle @if($state && $state == 'encours' && !$attente ) btn-outline-warning @else btn-outline-light @endif" type="button">En cours {{$all_commands->where('livreur_id', '!=', 11)->where('etat', 'encours')->count()}}</button>
        
          <button id="enattente_btn" style="font-size: 11px" class="mx-auto font-weight-bold text-uppercase btn btn-sm align-middle @if($attente && $attente == 'attente') btn-outline-warning @else btn-outline-light @endif " type="button">En attente {{$all_commands->where('livreur_id', 11)->where('etat', '!=', 'annule')->where('etat','=', 'encours')->count()}}</button>
          
          <button id="annule_btn" style="font-size: 11px" class="mx-auto font-weight-bold text-uppercase btn btn-sm align-middle @if($state && $state == 'annule') btn-outline-warning @else btn-outline-light @endif" type="button">
  ...
  Installer l'application Jibiat</button>
      ...
      <button style=" font-weight: bold; text-transform: uppercase;" class="btn btn-secondary  dropdown-toggle " type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
      Choisir un livreur 
      </button>
      ...
      @csrf
      <div class="input-group md-form form-sm form-1 pl-0 date">
      ...
      <button type="button" class="btn btn-primary " onclick="setGeoloc()" >Définir comme votre adresse de ramassage</button>
      </div>
     ...
     @endforeach
      ...
      <button style="font-weight: bold; background: #CAAA06" class="text-white  btn btn-light btn-block">choisir livreur par Zone</button>
      @foreach($commands as $command)
  <div  class="card target" style="border-style: solid; border-width: 2px; width: 100%">
      <ul  class="list-group list-group-flush" >
         
      <li  class=" list-group-item" >
      <div ><strong style="font-size: 30px">#{{$command->id}}</strong> <span 
      ...
      <button style="font-size: 9px; font-weight: bold; text-transform: uppercase;"  data-cur="ready_{{$command->id}}"  data-sp="unready2_{{$command->id}}" value="no" data-id="{{$command->id}}"  class=" ready btn btn-primary btn-sm">Pas prêt?</button>
      </span>
      <span   id="ready_{{$command->id}}" >
      <button style="font-size: 9px; font-weight: bold; text-transform: uppercase;"  data-sp="ready_{{$command->id}}" data-cur="unready2_{{$command->id}}" value="yes" data-id="{{$command->id}}" class=" ready btn btn-primary  btn-sm">Commande Prête?</button>
      </span>
      @else
      <span hidden id="ready2_{{$command->id}}" >
      <button style="font-size: 9px; font-weight: bold; text-transform: uppercase;" data-cur="unready_{{$command->id}}" data-sp="ready2_{{$command->id}}" value="yes"  data-id="{{$command->id}}" class=" ready btn btn-primary btn-sm">Commande Prête?</button>
      </span>
      <span id="unready_{{$command->id}}"  >
      <img width="30" height="30" src="/assets/img/packing.ico">
      <button style="font-size: 9px; font-weight: bold; text-transform: uppercase;" data-cur="ready2_{{$command->id}}" data-sp="unready_{{$command->id}}" value="no" data-id="{{$command->id}}"  class=" ready btn btn-primary btn-sm">Pas prêt?</button>
      </span>
      @endif
   </span>
   ...
   @if($command->note->count()>0)
      <a style="color: orange" data-toggle="modal" data-target="#noteViewModal{{$command->id}}" href=""> <i class="fa fa-sticky-note" >Note de livraison</i></a>
      @endif
   <br>
      ...
      <button style=" font-weight: bold; text-transform: uppercase; background: #CAAA06; color: white" class="btn    dropdown-toggle " type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span  hidden="hidden" class="spinner-border spinner-border-sm spinner{{$command->id}}" role="status" aria-hidden="true"></span><span class="sr-only"></span>
      <img src="../assets/img/moto.png" width="30px" height="30px">Choisir un Livreur 
      </button>
...
<span class="dropdown-menu dropdown-menu-right " aria-labelledby="dropdownMenuButton">
      ...
      @endif
       ...
       <a   class="dropdown-item"><i class="fa fa-phone"></i>Appeler livreur</a>     
   ...
   <div id="payModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
      ...
      <h2 class="modal-title">Liste des payements</h2>
      </div>
      <div >
...
<div id="znModal" class="modal fade" role="dialog">
    <div class="modal-dialog">
      <!-- Modal content-->
      <div class="modal-content">
      <button type="button" class="close float-left" data-dismiss="modal">&times;</button>
   ...
   <button type="button" class="btn btn-default float-right" data-dismiss="modal">Fermer</button>
    ...
    <div class="modal-dialog">
      ...
      <h2 class="modal-title ">Liste des commandes non livrés</h2>
      ...
      Revenir à la liste </button></span><br>
...
<div  id="stateModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div  class="modal-content">
      ...
      <div  id="stateModalBody" class="modal-body">
...
<div  id="billModal" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div  class="modal-content">
      ...
      <div  id="billModalBody" class="modal-body">
 ...
 <nav style="margin-bottom: 0; " class="fixed-bottom navbar navbar-expand-lg navbar-light bg-light">
  ...
  <a href="#" style="text-align: center; color: black; font-size: 12px" class="cmdRtrn">
...
<a  href="#" style="text-align: center;color: black; font-size: 12px" data-toggle="modal" data-target="#ncModal">
...
Nouvelle <br>
 Commande
   ...
   <a href="#" class="pay"  style="text-align: center;color: black; font-size: 12px" >
...
<script type="text/javascript">
   ...
   error: function(response){
    ...
    $(".zn_assign").click( function() {
   
  
   $("#znModal").modal("show");
   
   $(".znSpinner").removeAttr('hidden');
   ...
   var current_state = $("#"+cur_stt);
     ...
     $.ajax({
       ...
       success: function(response){
         ...
         $("#stateModalBody").html('<img width="60px" height="60px" src="/assets/img/packing.ico">'+response.message);
