 ...
 $id = $request->cmd_id;
  ...
  $model = Command::findOrFail($id);
   ...
   {$message = "Commande récupérée";
    ...
    if($model->client->id == 8)
    {$message = "Votre Commande numero".$model->id. " ".$model->description. " à été récupérée. "."livreur:".$model->livreur->nom. " ".$model->livreur->phone. ". Vendeur:".$model->client->nom. " ". $model->client->phone;
...
$payment->save();
  ...
  return response()->json(['message'=>$message, 'total_pending'=> $total_pending, 'route_id'=>$route_id]);
...
public function livraisons(Request $request)
{
  ...
  $day = "Aujourd'hui";
      ...
      $etats_chauds = array('encours', 'en chemin', 'recupere');
      $notes = array("Ne décroche pas", "Injoignable", "En déplacement", "Décommandé", "Reporté par le client", "Indisponible", "Promesse de rappeler", "Article réfusé", "Client déja livré", "RDV", "Erreur sur l'article", "Livreur sur place, ne décroche pas");
