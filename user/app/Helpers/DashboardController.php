...
$cmd_by_zone  = Command::selectRaw('COUNT(id) nbre, (fee_id) fee_id')
 ...
 $livreurs = Livreur::where('status', 'active')->where('id', '!=', 9)->where('id', '!=', 20)->where('id', '!=', 81)->where('city', '!=', $client->city) ;

$client_city_livreurs = Livreur::where('status', 'active')->where('id', '!=', 9)->where('id', '!=', 20)->where('id', '!=', 81)->where('city', $client->city)->get();
...
if($request->input('my_liv'))
 ...
 $fees = Fee::all();
 return view('livreurs')->with('livreurs', $livreurs)->with('client', $client)->with('zone', $zone)->with('id', $id)->with('client_commands', $client_commands)->with('cmd_by_zone', $cmd_by_zone)->with('fees', $fees)->with('verify', $verify)->with('client_verify', $client_verify)->with('client_city_livreurs', $client_city_livreurs);
}
...
return view('daily')->with('commands_by_date', $commands_by_date)->with('livreur', $livreur)->with('done_by_date',$done_by_date)->with('undone_by_date', $undone_by_date)->with('livreurs', $livreurs)->with('unpayed_by_date', $unpayed_by_date)->with('payments_by_livreurs', $payments_by_livreurs)->with('done_mt_by_date', $done_mt_by_date)->with('commands_nbre_by_date', $commands_nbre_by_date);
 ...
 // Update record
  public function assigncommand(Request $request){
    ...
    $command->update();
    $cur_liv = substr($livreur->nom, 0, 10).".";
      ...
      if($livreur->commands->count()>0)
      {
       ...
       if(count($liv_in_zone) > 0)
       {
         ...
         foreach($zone_livreurs as $zn_lv)
         {
          ...
          $zone_output .= "<div><button style='width:80%' type='button' class='assign btn btn-outline-primary' data-mdl='LivChoice".$command->id."' data-cur='cur_liv".$command->id."' data-id='".$command->id. "' value='".$zn_lv->id. "'  > ".$zn_lv->nom. "<br> <strong style='font-size:10px;' > ".$remaining. " livraison(s) encours actuellement</strong> </button><a style=' font-weight: bold;' href='tel:".substr(preg_replace('/[^0-9]/', '',$zn_lv->phone), 0, 8)."' class='btn btn-primary'>
                 <ion-icon name='call-outline'></ion-icon></a><br></div><br>";
        }
       ...
       foreach($other_livreurs as $ot_lv)
         {
          ...
          $other_output .= "<div>
          <button style='width:80%' type='button' class='assign btn  btn-outline-primary' data-mdl='LivChoice".$command->id."' data-cur='cur_liv".$command->id."' data-id='".$command->id. "' value='".$ot_lv->id. "'  > ".$ot_lv->nom. "<br> <strong style='font-size:10px'>  ".$ot_remaining ." livraison(s) encours actuellement</strong> </button><a type='button' style=' font-weight: bold;' href='tel:".substr(preg_replace('/[^0-9]/', '',$ot_lv->phone), 0, 8)."' class='btn btn-primary '>
                 <ion-icon name='call-outline'></ion-icon></a></div><br>";}
...
public function showlivreurs(Request $request){
    $client_id = Auth::user()->client_id;
     ...
     foreach($livreurs as $livreur)
     {
      ...
      $output .= "<div>
    <button style='width:80%' type='button' class='assign btn  btn-outline-primary' data-ids='".implode(",", $cmd_ids). "' value='".$livreur->id. "' data-nom='".substr($livreur->nom, 0, 10).".' > ".$livreur->nom. "<br> <strong style='font-size:10px'>  ".$ot_remaining ." livraison(s) encours actuellement</strong> </button><a type='button' style=' font-weight: bold;' href='tel:".substr(preg_replace('/[^0-9]/', '',$livreur->phone), 0, 8)."' class='btn btn-primary '>
                 <ion-icon name='call-outline'></ion-icon></a></div><br>";
     }
    ...
    $.ajax({
      ...
      success: function(response){
        if(response.livreur_id == 11)
         ...
         {
          $('#stateModalBody').html('Commandes assignées à '+livreur_nom);

          
                 (current_modal).modal('hide');
                 for (i = 0; i < cur_livs.length; i++) {
               $('#cur_liv'+cur_livs[i]).html(livreur_nom);
}
