...
public function payall(Request $request){
   ...
   $client = Client::findOrFail($client_id);
    ...
    foreach($commands as $command){
      $command->cashedback_to = Auth::user()->name . "_". Auth::user()->phone;
      $command->cashedback_key = $nom. "_".$cashedkey;


      $command->update();

      $command->cashedback_at = $command->updated_at;
      
      $command->update();


      $route_montant[] = $command->montant-$command->remise;
      $route_ids .= $command->id. ",";
      $command->payment->etat = 'termine';
      $command->payment->payment_method = $method;
      $command->payment->update();
    ...
    $payed_commands =  Command::where("cashedback_key", $nom. "_".$cashedkey)
     ->where("etat", "termine")
      ->where('cashedback_at', "!=", null)
     ->where('montant', '>', 0)
    ...
    $singleinfo =  Command::where("cashedback_key", $nom. "_".$cashedkey)
     ->where("etat", "termine")
      ->where('cashedback_at', "!=", null)
     ->where('montant', '>', 0)
    ...
    ->first();
     ...
     return redirect()->back()->with("status", "Versement effectué")->with("payed_commands", $commands)->with("singleinfo", $singleinfo);
  }
...
public function cancelpay(Request $request){
    ...
    $commands = Command::where("cashedback_key", $cashedkey)
                   
                       ->get();

    foreach($commands as $command){
      $command->cashedback_to = NULL;
      $command->cashedback_key = NULL;
      $command->cashedback_at = NULL;
   ...
   $nom = $client->nom;
    ...
    $route = new Lesroute;
    $route->action = "PAYCANCEL";
    $route->observation = $route_ids;
    $route->montant = array_sum($route_montant);
    $route->client_id = $client_id;
    $route->client_nom = $client->name;
    $route->payed_by = Auth::user()->name. "_". Auth::user()->phone;
    ...
    $route->save();
     ...
     return redirect()->back()->with("status", "Versement Annulé");
  }



  public function cancelmoneypay(Request $request){
    ...
    $commands = Command::where("cashkey", $cashedkey)
                   
                       ->get();

    foreach($commands as $command){
      $command->payed_to = NULL;
      $command->cashkey = NULL;
      $command->payed_at = NULL;
   ...
   $nom = $client->nom;
    ...
    $route->save();
     ...
     return redirect()->back()->with("status", "encaissement Annulé");
  }
...
$payment_roles = $roles->checkrole("PAYMENT", $client->client_type);
 ...
 $recette = Command::selectRaw('SUM(livraison) livraison, SUM(montant-remise) montant, (livreur_id) livreur_id')
  ...
  return view("payements")->with("payed_by_client",$payed_by_client)->with("unpayed", $unpayed)->with('livreurs', $livreurs)->with("payed_by_livreur", $payed_by_livreur)->with("day",$day)->with("recette", $recette)->with("clients",$clients)->with("payment_roles", $payment_roles);
  }

  public function payedbylivreur(Request $request){
    $client = Client::findOrFail($request->id);
    $unpayed = Command::selectRaw('SUM(montant) montant, (livreur_id) livreur_id')
