...
class DashboardController extends Controller
{
  ...
  public function phonecheck(Request $request){
    ...
    $phone_check = Command::where('phone', str_replace(' ', '',$request->phone))->where("client_id", $client_id)->where('delivery_date', today())->get();
     
            if($phone_check->count()>0)
             {
                $result = "Ce numero à été utilisé pour ". $phone_check->count() ." commande(s) aujourd'hui";
 ...
 public function fees()
    ...
    $fee_roles = $roles->checkrole("TARIF", $client->client_type);
       $fees = Fee::with("command")->with("tarifs")->orderBy("destination")->get();

        
        
        return view('fees')->with('fees', $fees->toJson() )->with("client", $client)->with("fee_roles", $fee_roles);
    ...
    $fee_roles = $roles->checkrole("TARIF", $client->client_type);
        ...
        return view('feeregister')->with("client", $client)->with("fee_roles", $fee_roles);
    ...
    public function feeregister(Request $request)
    {
       ...
       return redirect()->back()->with('status', "Tarif Ajouté");
     ...
     public function feeedit($id)
    {
      ...
      $client = Auth::user()->client;
    $fee_roles = $roles->checkrole("TARIF", $client->client_type);
       return view('feeedit')->with('fee', $fee)->with("client", $client)->with("fee_roles", $fee_roles);
    }
...
public function photoupload(Request $req){
   ...
   public function ratelivreur(Request $request)
  {
   ...
   $average = $livreur->AverageRating;
   $livreur->average = $average;
   $livreur->update();
   response()->json([]);
  }
...
$sales_by_zone = Command::selectRaw('SUM(montant) montant, (fee_id) fee_id')
                ->where('etat', 'termine');
$sales_by_salesman = Command::selectRaw('SUM(montant) montant, (client_id) client_id')->where('etat', 'termine'); 
...
$sales_by_salesman = $sales_by_salesman->groupBy('client_id')->orderBy('montant', 'desc')->get();
  ...
  return view('point')->with('sales', $sales)->with('sales_by_zone', $sales_by_zone)->with('all_fees', $all_fees)->with('client', $client)->with("title", $title)->with("sales_by_salesman", $sales_by_salesman);
}
...
$livreurs = $livreurs->where("certified_at", "!=", "NULL")->orderBy("experience", "desc")->orderBy("average", "desc")->paginate(20);
