 ...
 data-toggle="modal" data-target="#depositActionSheet" class="float-right btn btn-success btn-rounded"><ion-icon name="add-outline"></ion-icon></button>
    ...
    {{$upcoming->montant}}F ({{$upcomings_count[$x]->nbre}})
     ...
     @if($payments_by_livreurs->count()>0 || $undone_by_livreurs->count()>0)
        <div class="section full mb-3">
            <div class="section-title"> <button  type="button" class="pay btn btn-sm btn-warning  btn-block" ><span class="float-left" style="font-weight: bold; color: black;">Payement: {{$payments_by_livreurs->sum("montant")}}f</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  <span class="float-right" style="font-weight: bold; color: black;">Colis Non livrés: {{$undone_by_livreurs->sum("nbre")}}</span> 
    </button></div>
             <div class="transactions">
            ...
            @if($livreur->commands->where("client_id", $client->id)->where('delivery_date', '>=', "2020-11-10")->whereIn("etat", ["recupere", "en chemin"])->count()>0 || $livreur->payments->where("client_id", $client->id)->where("etat", "en attente")->sum("montant")>0)
                    ...
                    <div class="right">
                        ...
                        <!-- <div class="position-absolute bottom-0 end-0">{{$loop->iteration}}/@if($payments_by_livreurs->count() > $undone_by_livreurs->count()){{$payments_by_livreurs->count()}} @else{{$undone_by_livreurs->count()}} @endif</div> -->
                    </div>
                </div>
