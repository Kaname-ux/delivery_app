               ...
               <div class="item target" id="{{$command->id}}">
                    <!-- card block -->
    ...
    <button class="btn btn-danger btn-sm del" id="del{{$command->id}}" value="{{$command->id}}">Supprimer</button>
    <button class="btn btn-success btn-sm activate" id="activate{{$command->id}}" value="{{$command->id}}"><ion-icon  name='power-outline'></ion-icon>Activer</button>
    
    @endif -->

    
    <button hidden class="btn btn-success btn-sm activate" id="activate2{{$command->id}}" value="{{$command->id}}"><ion-icon  name='power-outline'></ion-icon>Activer</button>
      ...
      @endif
          ...
          data-etat="{{$command->etat}}" data-livphone ="{{$command->livreur->phone}}" data-total="@if($command->livraison != NULL) {{$command->montant+$command->livraison}}  @else {{$command->montant+$command->fee->price}} @endif" @if($command->livraison == NULL) data-liv="no" @else data-liv="{{$command->livraison}}" @endif
                   ...
                   type="button" class="btn  text-white btn-icon float-right cmd_menu" id="cmd_menu{{$command->id}}" >
