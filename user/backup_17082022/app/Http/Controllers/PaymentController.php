 ...
 $(\".singlePayCancel\").click(function(){
  ...
  foreach($payments as $payment){
   ...
   $payarea = "";
    ...
    <ul class='list-group list-group-flush' id='cmd_id".$payment->command->id."'><li  class='pt-6 list-group-item'><strong>#" .$payment->command->id. "</strong><span id='cmd_state".$payment->command->id."' ". $class .">".$txt ." ".$payment->command->updated_at->format('d-m-Y H:i:s'). "</span><br> ". $payment->command->montant. "<br> ".$payment->command->description. "<br>".$payment->command->adresse ." ". "<span id='paystate$payment->command->id' >$pay_state</span><br>$note<br>
      $payarea
