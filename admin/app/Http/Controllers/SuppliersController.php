...
class SuppliersController extends Controller
{
    public function suppliers(Request $request)
    {
      ...
      $filter .=  "<div>Filtre Pays: ";
      $suppliers = $clients->whereIn('country', $request->fees);
      
      
      foreach($request->countries as $country){
        

        $filter .= "<strong class='text-dark'>". $country. "</strong>. ";  
      }
      $filter .=  "</div>";
    }

    if(!empty($request->products)){
      $products = Product::whereIn("id", $request->products)->toArray();
      $filter .=  "<div>Filtre Produits: ";
      $suppliers = $suppliers->whereIn('id', $products['supplier_id']);
      
      
      foreach($products['name'] as $product){
        

        $filter .= "<strong class='text-dark'>". $product. "</strong>. ";  
      }
      $filter .=  "</div>";
    }
     ...
     if($request->search_term){
        if($request->search_term != ""){
            ...
            if($request->search_type == "email"){
                $suppliers = $suppliers->where("email",  $request->search_term);
            }

            if($request->search_type == "name"){
               $suppliers = $suppliers->where("name", 'like', '%' . $request->search_term . '%' );
            }
        }
...
array('716', 'ZWE', 'ZW', 'Zimbabwe')
        ...
        return view('suppliers')->with('suppliers', $suppliers)->with('sups', $suppliers->toJson())->with("products", $products)->with("filter", $filter)->with("search_result", $search_result)->with("countries", $countries);
    ...
    public function create(Request $request){
         $validatedData = $request->validate([
            ...
            'contact' => 'required| string| max:10| min:10|unique:suppliers',
            'whatsapp' => 'string| max:10| min:10|unique:suppliers',
         ...
         $supplier->contact = preg_replace('/[^0-9]/', '', $request->contact);
         $supplier->whatsapp = preg_replace('/[^0-9]/', '', $request->whats);
         ...
         return redirect()->back()->with("status", "Fournisseur enregistre");
    }
    ...
    public function edit(Request $request){
        ...
        Rule::unique('suppliers')->ignore($request->id),
         ...
         return redirect()->back()->with("status", "Fournisseur modifie");
    }
}
