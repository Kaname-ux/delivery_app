...
return response()->json(["livreurcmds"=>$livreurcmds]);
   ...
   $event->description = "Prix modifié. Precedent: ".$previouscost.". Modification :".$montant;
  ...
  return response()->json(["montant"=> $command->montant]);
  ...
  public function updateliv(Request $request){
   ...
   $event->description = "Cout livraison modifié. Precedent : ".$previousliv.". Modification: ".$livraison;
  ...
  return response()->json(["livraison"=> $command->livraison]);
 ...
 $event = new Command_event;
  ...
  public function updateclient(Request $request){
       ...
       $cmdid = $request->cmdid;
        ...
        $previousclient = $command->costumer;

        

        $previousadresse = $command->adresse;

        $previousphone1 = $command->phone;

        $previousphone2 = $command->phone2;
       ...
       if($request->client)
...

        {
         ...
         if($request->adresse){
          ...
          $actual_fee = $command->fee->destination;

          $command->adresse = $actual_fee.":".$request->adresse;
...
$command->update();
 ...
 $event = new Command_event;
   ...
   $event->description = "Client modifié. Precedent: ".$previousclient.". ".$previousadresse . ". ". $previousphone1. ". ".$previousphone2 . ". Modification: ".$command->costumer.". ".$command->adresse . ". ". $command->phone. ". ".$command->phone2;
  ...
  return response()->json(["client"=> $command->nom_client, "fee"=> $command->fee, "adresse"=>$command->adresse, "phone"=> $command->phone, "phone2"=> $command->phone2]);
...
public function commands(Request $request)
    ...
    $filter = "";
      ...
      $clients = $all_clients->where("client_type", "!=", null)->get();



      $providers = $all_clients->where("client_type", null)->orWhere("client_type", "CLIENT")->get();
...
if($request->start && $request->end )
    ...
    ->get();
     ...
     $upcomings = Command::selectRaw('SUM(livraison) montant, (delivery_date) delivery_date')
...
$upcomings_count = Command::selectRaw('COUNT(montant) nbre, (delivery_date) delivery_date')
   ...
   if($request->fees || $request->livreurs || $request->sources || $request->clients || $request->providers || $request->etats || $request->assigns){
    ...
    if(!empty($request->clients)){
      ...
      $commands = $commands->whereIn('user_id', $request->clients);

      

      $cmds_by_livreur = $cmds_by_livreur->whereIn('user_id', $request->clients);

      foreach($request->clients as $clt){

        $filteredclient = User::findOrFail($clt);



        $filter .= "<strong class='text-dark'>". $filteredclient->name. "</strong>. ";  
 ...
 if(fmod($all_commands->count(), $limit) > 0){
  $pages += 1;
     ...
     $nbre = $commands->count('montant');
      ...
      $fees = Fee::with("tarifs")->orderBy('destination', 'asc')->get();
      ...
      $certifications = Certification::where("status", "pending")->get();
       ...
       $categories = Product_category::get();
        ...
        return view('commands')->with("filter", $filter)->with('commands', $commands->toJson())->with('day', $day)->with('client', $client)->with('fees', $fees)->with('total', $total)->with("phone_check", $phone_check)->with('livreurs', $livreurs)->with('deliverymen', $livreurs->toJson())->with('id', $id)->with('all_commands', $all_commands)->with('payments_by_livreurs', $payments_by_livreurs)->with('detail', 'tout')->with('encours_states', $encours_states)->with('nbre', $nbre)->with('state', $state)->with('attente', $attente)->with('undone_by_livreurs', $undone_by_livreurs)->with('cmds_by_livreur', $cmds_by_livreur)->with('current_date', $current_date)->with('chk', $chk)->with('upcomings', $upcomings)->with('certifications', $certifications)->with("start", $start)->with("end", $end)->with("products", $products->toJson())->with("filteredfees", $filteredfees)->with("filteredlivreurs", $filteredlivreurs)->with('upcomings_count', $upcomings_count)->with('allfees', $fees->toJson())->with('srces', $sources->toJson())->with('sources', $sources)->with("clients", $clients)->with('categories', $categories->toJson())->with("users", $clients->toJson())->with("providers", $providers)->with("bclients", $base_clients->toJson())
          ->with("pages", $pages)->with("page", $page)->with("limit", $limit)->with("begining", $begining);
...
public function commandfastregister(Request $request)
  ...
  if($request->start && $request->end)

   {
      ...
      $begining = ($request->page-1)*50;
     

    $commands = Command::offset($request->page*50)
    ...
    if($smscount > 0){
      ...
      $codifieds = array("NUMERO_CMD"=>$model->id, "TOTAL_CMD"=>$model->montant, "TRACKING_CMD"=>'https://user.logisticadelivery.com/tracking/'.$model->id, "LIVREUR_CMD"=>$livreur_info);
  ...
  public function assign(Request $request){
   ...
   $livreur_info = "";
    ...
    $notify = "Nouvelles commandes assignées";               
      ...
      event(new newAssignment($notify));
