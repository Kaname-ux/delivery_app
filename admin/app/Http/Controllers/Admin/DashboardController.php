...
class DashboardController extends Controller
{
   ...
   public function actions(Request $request) 
  {
      ...
      $actions = Lesroute::where("livreur_id", $id)->orderBy("created_at", "desc");
      $day = date("Y-m-d");
      $action_date = "Aujourd'hui";
     if($request->action_date != 'null')
  { 
    $day = date_format(date_create($request->action_date), 'Y-m-d');
    $action_date = date_format(date_create($request->action_date), 'd-m-Y');
    
   }

   $actions = $actions->whereDate("created_at", $day)->get();
   ...
   if($actions->count()>0)
     {foreach($actions as $x=> $action)
              ...
              {$output .=   "<i  class='fas  fa-circle text-success'></i>". $action->created_at->format("d-m-Y H:i").  "- <span class='text-primary'>".$action->observation." </span>";
                           if($action->latitude != null && $action->longitude != null)
                           
                             { $output .=   "<br><ion-icon name='navigate-outline'></ion-icon>".  app('geocoder')->reverse($action->longitude,$action->latitude)->get();
                              }
                             
                          
                   
                           if($x + 1 < $actions->count())
                             
                              { $output .=   "<div style='text-align: center;'>
                                                   
                                                   <ion-icon name='arrow-up-outline'></ion-icon><br>
                                                    
                                                   <strong>". LivreurHelper::actionint($action->created_at, $actions[$x+1]->created_at). "</strong>
                                               </div>";
                  ...
                  $output .= "<br><br>";
                    ...
                    $output .=   "Aucune action enregistrée.";
                   }
        ...
        return response()->json(["output"=>$output, "action_date"=>$action_date]);
...
if($request->start && $request->end )
     ...
     if($start == $end)
       ...
       elseif($start == date('Y-m-d',strtotime("-1 days")))
       {
        ...
        $commands = Command::whereBetween("delivery_date", [$start, $end])->with("client")->get();
...
$cmds_by_client = Command::selectRaw("COUNT(montant) qty, (client_id) client_id")
                      ->whereBetween('delivery_date', [$start, $end])
                      ->with(['client' => function($q) {
                       $q->where('client_type', null);}])
                      ->orderBy("qty", "desc")
                      ->groupBy("client_id")
                      ->get();


$cmds_by_city = Command::selectRaw("COUNT(montant) qty, (fee_id) fee_id")
                      ->whereBetween('delivery_date', [$start, $end])
                      ->orderBy("qty", "desc")
                      ->groupBy("fee_id")
                      ->get();                      
        ...
        return view('dashboard')
             ->with("cmds_by_client", $cmds_by_client)
             ->with("cmds_by_city", $cmds_by_city)
            
            ->with("commands", $commands)

            ->with("notes", $notes)->with("livreurs", $livreurs)
    ...
    public function refreshnotes(Request $request){
        $start = $request->start;
        $end =$request->end;
       ...
       $commands = Command::whereBetween('delivery_date', ["start", "end"])->with("products")->get();
...
public function clientdelete($id)
   ...
   if ($done_livraisons->count()> 0) {
     ...
     public function fees()
    {
       ...
       $fees = Fee::with("command")->with("tarifs")->orderBy("destination")->get();

        
        
        return view('fees')->with('fees', $fees->toJson() );
...
public function repportstart(){
    $subscribers = Subscription::whereBetween("created_at", [date("Y-m-d",strtotime("first day of this month")), date("Y-m-d",strtotime("last day of this month"))])->get();
    $commands = Command::whereBetween("created_at", [date("Y-m-d",strtotime("first day of this month")), date("Y-m-d",strtotime("last day of this month"))])->where("subscription_id", null)->get();

    return view("repportstart")->with("subscribers", $subscribers)->with("commands", $commands);
}
...
public function rapports(Request $request)
    {
      ...
      $januarycmds = Command::whereBetween('delivery_date', [date("Y-01-01"), date("Y-01-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");


      $februarycmds = Command::whereBetween('delivery_date', [date("Y-02-01"), date("Y-02-29")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $marchcmds = Command::whereBetween('delivery_date', [date("Y-03-01"), date("Y-03-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $aprilcmds = Command::whereBetween('delivery_date', [date("Y-04-01"), date("Y-04-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $maycmds = Command::whereBetween('delivery_date', [date("Y-05-01"), date("Y-05-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");
     
      
      $juncmds = Command::whereBetween('delivery_date', [date("Y-06-01"), date("Y-06-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $julycmds = Command::whereBetween('delivery_date', [date("Y-07-01"), date("Y-07-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");


      $augustcmds = Command::whereBetween('delivery_date', [date("Y-08-01"), date("Y-08-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $septembercmds = Command::whereBetween('delivery_date', [date("Y-09-01"), date("Y-09-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $octobercmds = Command::whereBetween('delivery_date', [date("Y-10-01"), date("Y-10-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $novembercmds = Command::whereBetween('delivery_date', [date("Y-11-01"), date("Y-11-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");
     
      
      $decembercmds = Command::whereBetween('delivery_date', [date("Y-12-01"), date("Y-12-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");


     //previous
      $pjanuarycmds = Command::whereBetween('delivery_date', [date($previousY."-01-01"), date($previousY."-01-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");


      $pfebruarycmds = Command::whereBetween('delivery_date', [date($previousY."-02-01"), date($previousY."-02-29")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $pmarchcmds = Command::whereBetween('delivery_date', [date($previousY."-03-01"), date($previousY."-03-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $paprilcmds = Command::whereBetween('delivery_date', [date($previousY."-04-01"), date($previousY."-04-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $pmaycmds = Command::whereBetween('delivery_date', [date($previousY."-05-01"), date($previousY."-05-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");
     
      
      $pjuncmds = Command::whereBetween('delivery_date', [date($previousY."-06-01"), date($previousY."-06-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $pjulycmds = Command::whereBetween('delivery_date', [date($previousY."-07-01"), date($previousY."-07-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");


      $paugustcmds = Command::whereBetween('delivery_date', [date($previousY."-08-01"), date($previousY."-08-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $pseptembercmds = Command::whereBetween('delivery_date', [date($previousY."-09-01"), date($previousY."-09-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $poctobercmds = Command::whereBetween('delivery_date', [date($previousY."-10-01"), date($previousY."-10-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");

      $pnovembercmds = Command::whereBetween('delivery_date', [date($previousY."-11-01"), date($previousY."-11-30")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");
     
      
      $pdecembercmds = Command::whereBetween('delivery_date', [date($previousY."-12-01"), date($previousY."-12-31")])->where('etat', 'termine')->where("subscription_id", null)->sum("livraison");
...
function days_between_dates($date1, $date2) {
  ...
  $clients_by_zone =  Client::selectRaw("COUNT(city) qty,  (city) city")
       ...
       $commands = Command::whereBetween('delivery_date', [$start, $end])->where('etat', 'termine')->where("subscription_id", null)->get();
   ...
   ->whereBetween('delivery_date', [$start, $end])->where("subscription_id", null)
   ...
   ->whereBetween('delivery_date', [$start, $end])->where("subscription_id", null)
    ...
    ->get();
     ...
     $command_by_clients_undelivered = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start, $end])
     ->where("etat", "annule")
     ->orderBy('montant', 'desc')
    ->groupBy('client_id')
   ...
   $command_by_livreurs = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
  ->whereBetween('delivery_date', [$start, $end])
    ...
    $command_by_livreurs_delivered = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
  ->whereBetween('delivery_date', [$start, $end])
   ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ->groupBy('livreur_id')
  ...
  $inactifs_by_zone = Fee::whereDoesntHave("commands", function($q) use($start, $end){
   ...
   ->with('commands')->selectRaw("COUNT(id) qty, (id) city")->groupBy("id")->get();
    ...
    $command_by_clients1 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ...
    $command_by_clients_delivered1 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ->whereBetween('delivery_date', [$start1, $end1])
  ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ...
    $command_by_clients_undelivered1 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ->whereBetween('delivery_date', [$start1, $end1])
  ->where("etat", "annule")
     ->orderBy('montant', 'desc')
    ->groupBy('client_id')->get()
   ...
   ->whereBetween('delivery_date', [$start1, $end1])->where("subscription_id", null)
   ...
   $command_by_livreurs1 = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start1, $end1])
    ...
    ->get();
     ...
     $command_by_livreurs_delivered1 = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start1, $end1])
  ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ->groupBy('livreur_id')
    ...
    $command_by_livreurs_undelivred1 = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start1, $end1])
  ->where("etat", "annule")
     ->orderBy('montant', 'desc')
    ->groupBy('livreur_id')
  ...
  $inactifs_by_zone1 = Fee::whereDoesntHave("commands", function($q) use($start1, $end1){
   ...
   ->with('commands')->selectRaw("COUNT(id) qty, (id) city")->groupBy("id")->get();
    ...
    $command_by_clients2 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ...
    $command_by_clients_delivered2 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ->whereBetween('delivery_date', [$start2, $end2])
     ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ->groupBy('client_id')
    ...
    $command_by_clients_undelivered2 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ->whereBetween('delivery_date', [$start2, $end2])
     ->where("etat", "annule")
     ->orderBy('montant', 'desc')
    ->groupBy('client_id')
   ...
   ->whereBetween('delivery_date', [$start2, $end2])->where("subscription_id", null)
   ...
   $command_by_livreurs2 = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start2, $end2])
    ...
    $command_by_livreurs_delivered2 = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start2, $end2])
  ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ->groupBy('livreur_id')
  ...
  $inactifs_by_zone2 = Fee::whereDoesntHave("commands", function($q) use($start2, $end2){
   ...
   //$day3 month
    ...
    $command_by_clients3 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ...
    $command_by_clients_delivered3 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ->whereBetween('delivery_date', [$start3, $end3])
     ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ->groupBy('client_id')
    ...
    $command_by_clients_undelivered3 = Command::selectRaw('SUM(livraison) montant, COUNT(montant) qty, (client_id) client_id')->where("subscription_id", null)
    ->whereBetween('delivery_date', [$start3, $end3])
     ->where("etat", "annule")
     ->orderBy('montant', 'desc')
    ->groupBy('client_id')
   ...
   ->whereBetween('delivery_date', [$start3, $end3])->where("subscription_id", null)
   ...
   ->whereBetween('delivery_date', [$start3, $end3])->where("subscription_id", null)
    ...
    $command_by_livreurs_delivered3 = Command::selectRaw('SUM(livraison) montant, COUNT(livraison) qty, (livreur_id) livreur_id')->where("subscription_id", null)
   ->whereBetween('delivery_date', [$start3, $end3])
  ->where("etat", "termine")
     ->orderBy('montant', 'desc')
    ->groupBy('livreur_id')
...
public function users(Request $request)
{
 ...
 if($request->search){
    ...
    $clients = $clients->where('nom','LIKE',"%{$search}%")
