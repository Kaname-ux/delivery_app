 ...
 <div class="input-group mb-3 livreurInput">
                        ...
                        <div class="action-sheet-content ">
                            <form action="?">
                                <div hidden>
                                    ...
                                    <input value="{{$page}}"  type="text" name="page">
                                </div>
                          ...
                          <div class="form-group">
                            <label class="form-label">Filtrer par utilisateur</label>
                              <select data-style="btn-dark" v-model="clients" title="Tous les utilisateurs" id="client-select" class=" selectpicker form-control" multiple  name="clients[]">
                                <option value="{{$client->id}}}">Moi-meme</option>
                                 @if($clients->count() > 0)
                                 @foreach($clients as $clt)
                                 @if($clt->user)
                                  <option value="{{$clt->user->id}}">{{$clt->nom}}</option>
                                  @endif
                                 @endforeach
                                 @endif
                                 </select>
  ...
  <div class="content-wrapper">
      ...
      <div class="container-fluid">
        <div class="row mb-2">
         ...
         @foreach($upcomings  as $x=>$upcoming)
            ...
            <a class="btn-group mb-2 mr-2" href="commands?state=all&start={{$upcoming->delivery_date->format('Y-m-d')}}&end={{$upcoming->delivery_date->format('Y-m-d')}}">
    ...
    {{$upcoming->montant}}F ({{$upcomings_count[$x]->nbre}})
        ...
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Mes courses <button data-toggle="modal" data-target="#dateModal" class="btn btn-outline-primary mr-2 d-print-none">{{$day}}</button></h1> 
    ...
    <section class="content">
      <div class="container-fluid">
        @if($start == $end)
        <div class="d-none d-print-block row mb-2"><h4>{{date_create($end)->format("d-m-Y")}}</h4></div> 
        @else
         <div class="d-none d-print-block row"><h4>Du {{date_create($start)->format("d-m-Y")}} au {{date_create($end)->format("d-m-Y")}}</h4></div>
...
<!-- 
        <h3>Total:</h3><h2 style=" font-weight: bold;">@{{ getTotal() }}(@{{commands.length}})</h2> &nbsp;&nbsp;<h3>/</h3>&nbsp;&nbsp;  
          <h3>Livres: </h3><h2 style="font-weight: bold;">@{{getLivre()[0]}}(@{{getLivre()[1]}})</h2> -->
            ...
            <a href="?start={{$start}}&&end={{$end}}" class="btn btn-primary d-print-none">Retour aux courses</a>
              ...
              <div  class="accordion " :id="'cmdProvider'+index">
                <div class="accordion-item">
                 ...
                 @foreach($providers as $provider2)
                  <option value="{{$provider2->id}}">{{ $provider2->nom}} {{$provider2->id}}</option>
                @endforeach
                  </select>
                    ...
                    <div :id="'accordion3'+index" class="accordion-body collapse d-print-none" :data-parent="'#cmdClient'+index">
                        ...
                        <div class="input-group-prepend">
                          ...
                          <select :id="'cmdFee'+index"   required  class="form-control" name="fee">
                             <option  value="">selectionner Une ville/commune</option>
                             @foreach($fees as $fee)
                            <option v-if="command.fee" :selected ="command.fee.id == '{{$fee->id}}'"
