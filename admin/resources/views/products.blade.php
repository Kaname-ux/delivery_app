...
@if($livreurs->count() > 0)
 <div class="input-group mb-3 livreurInput">
    ...
    <span class="input-group-text" id="">Livreur</span>
      ...
      <input v-model="observation" id="comobservation" id="comobservation" maxlength="150" value="{{ old('observation') }}"  name="observation" class="form-control " type="text" placeholder="Information supplementaire">
      </div>
...
<script type="text/javascript">
    ...
    data() {
        return {
            ...
            productCost:"{{old('cost')}}",
            productConditioning:"{{old('conditioning')}}",
            productSticker:"{{old('sticker')}}",
            productAd:"{{old('ad')}}",
            productCode:"{{old('code')}}",
            productPort:"{{old('port')}}",
            productCategory:"{{old('category')}}",
            productShop:"{{old('shop')}}",
            productSupplier:"{{old('supplier')}}",
            productId:"{{old('id')}}",
            otherFee:"",
            remise:"",
