  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{$user->name}}  <button data-toggle="modal" data-target="#dateModal" class="btn btn-outline-primary mr-2 d-print-none">{{$day}}</button></h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="/userstat?start={{$start}}&end={{$end}}">Liste</a></li>
    ...
    <section class="content">
        ...
        <div class="card">
          <div class="card-header"><h2 class="card-title">Objectifs et chiffres</h2>
           <div class="card-tools">
               ...
               Objectif de la periode: <span style="font-weight: bold;">{{$daily*$dif}}</span>
               ...
               <div class="row">
                <div class="col">
                    Enregitré: <span style="font-weight: bold;">{{$registered->count()}}</span>
                </div>
                <div class="col">
                       <div class="progress">
      @if($registered->count() > 0)
       <div class="progress-bar 
       @if(round(($registered->count()/($daily*$dif))*100) < 50) bg-danger @endif

       

       @if(round(($registered->count()/($daily*$dif))*100) >= 100) bg-success @endif


       " role="progressbar" style="width: {{round(($registered->count()/($daily*$dif))*100)}}%;" aria-valuenow="{{round(($registered->count()/($daily*$dif))*100)}}" aria-valuemin="0" aria-valuemax="100">{{round(($registered->count()/($daily*$dif))*100)}}%</div>
       @endif
       </div>   
                </div>
            ...
            <div class="row">
                ...
                <div class="col">
                   Livré: <span style="font-weight: bold;">{{$registered->where("etat", "termine")->count()}}</span>  
                </div>
      ...
      @if($registered->count() > 0)
       <div class="progress-bar 
       @if(round(($registered->where('etat', 'termine')->count()/$registered->count())*100) < 50) bg-danger @endif

       

       @if(round(($registered->where('etat', 'termine')->count()/$registered->count())*100) >= 100) bg-success @endif


       " role="progressbar" style="width: {{round(($registered->where('etat', 'termine')->count()/$registered->count())*100)}}%;" aria-valuenow="{{round(($registered->where('etat', 'termine')->count()/$registered->count())*100)}}" aria-valuemin="0" aria-valuemax="100">{{round(($registered->where('etat', 'termine')->count()/$registered->count())*100)}}%</div>
       @endif
       </div>  
            </div>
   ...
   <hr>
          <div class="row">
                ...
                <div class="col">
                   Annulé: <span style="font-weight: bold;">{{$registered->where("etat", "annule")->count()}}</span>  
                </div>
       ...
       " role="progressbar" style="width: {{round(($registered->where('etat', 'annule')->count()/$registered->count())*100)}}%;" aria-valuenow="{{round(($registered->where('etat', 'annule')->count()/$registered->count())*100)}}" aria-valuemin="0" aria-valuemax="100">{{round(($registered->where('etat', 'annule')->count()/$registered->count())*100)}}%</div>
       @endif
       </div>  
            </div>
                ...
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                    ...
                    <td>{{$action->created_at->format('d-m-Y H:i:s')}}</td>
                      <td>Commnade numero {{$action->id}} - {{$action->adresse}}</td>
                      
                     <td>
                      <a data-toggle="modal" data-target="#eventViewModal" href="#" @click="getEvent({{$action->id}})"> 
                       {{$action->events->count()}} modification(s)
                     </a>
                     </td>
                    </tr>
                    @endforeach
